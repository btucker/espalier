...
/// Espalier whose `espalier notify` CLI reports "not listening" — a
...
enum NotifySocketBanner {
    ...
    static func presentIfNeeded(error: SocketServerError) {
        ...
        alert.informativeText = """
            Espalier couldn't start its control socket, so `espalier notify`, \
            `espalier pane list`, `espalier pane add`, and `espalier pane close` \
            won't reach this instance. Terminals still work.

            Underlying error: \(Self.describe(error))

            Try quitting and relaunching Espalier. If the problem persists, \
            `ESPALIER_SOCK` may be set to an unwritable location — unset it \
