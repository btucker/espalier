...
struct SidebarView: View {
    ...
    private func worktreeBlock(_ worktree: WorktreeEntry, repo: RepoEntry) -> some View {
        ...
        // PWD-1.5: drop-target highlight. Distinct from `isActive`'s
        // background tint — uses a stroked outline at the same corner
        // radius (6) so the two cues can compose if the active worktree
        // is also being dragged onto.
        .overlay(
            RoundedRectangle(cornerRadius: 6, style: .continuous)
