...
- `↓<N>` when the base ref has N commits not reachable from the worktree's HEAD (behind). Omitted when N is zero. The behind segment shall be rendered in red so incoming origin commits — the only event `DIVERGE-4.3` fetches are supposed to surface — draw the eye, in contrast to the muted `↑` segment.
...
**DIVERGE-3.0** Divergence shall be measured against the **union** of upstream refs so that both a PR merge on the default branch and a collaborator push to the worktree's own branch surface as ↓N — on every worktree that doesn't yet have those commits. The upstream set is resolved per worktree as:
- `origin/<defaultBranch>` — always included when the repo has a resolvable default branch. This is the PR-merge signal: any merge advancing the default branch surfaces on every worktree, regardless of which branch that worktree is checked out on, because the merge's commits are now reachable from `origin/<defaultBranch>` but not yet from the worktree's HEAD.
- `origin/<worktree-branch>` — additionally included when the tracking ref exists (`git show-ref --verify` succeeds). This is the collaborator-push signal: teammate commits landing on the worktree's own upstream show up even when the default branch is unchanged.

The degenerate case `branch == defaultBranch` (home checkout on the default branch, or any worktree on the default) collapses to a single ref so `rev-list` isn't handed the same ref twice and the tooltip doesn't render `"origin/main + origin/main"`. The never-pushed-branch case (no `origin/<branch>` tracking ref) also collapses to just the default — `↓N` still catches PR merges on main, which is usually what the user wants to see on a scratch worktree.

The repo's default branch name is the one `GitOriginDefaultBranch.resolve` returns per `DIVERGE-2.x` (`origin/HEAD` symbolic-ref or the `main/master/develop` probe); there shall be no literal `"main"` anywhere in the compute path.

Pre-fix, the linked-worktree rule was always "local `<defaultBranch>`", which made `↓N` inert for feature-branch worktrees because the local default branch is only moved by the user — no amount of `git fetch` could surface a teammate's push to `origin/<feature>` *or* a PR merging into `origin/main`, since the comparison never looked at either origin ref.

**DIVERGE-3.1** The application shall compute the behind count by running `git rev-list --count <ref1> [<ref2>] ^HEAD` — commits reachable from any upstream ref but not HEAD — and the ahead count by running `git rev-list --count HEAD ^<ref1> [^<ref2>]` — commits reachable from HEAD but not from any upstream ref. `<ref1>`/`<ref2>` are the upstream refs resolved per `DIVERGE-3.0`. `rev-list` natively dedupes, so a commit reachable from both upstream refs (e.g. a merge commit that origin/feature was rebased onto) is counted once in the behind total.

The pre-fix shape (`rev-list --left-right --count <base>...HEAD`) was a symmetric-difference shortcut that works only for one ref on each side; it can't express a union of refs without double-counting the overlap.

**DIVERGE-3.2** The application shall compute insertion and deletion line counts by running `git diff --shortstat <ref>...HEAD` in the worktree directory, where `<ref>` is `origin/<worktree-branch>` when that tracking ref exists, otherwise `origin/<defaultBranch>`. This measures "your commits on this branch" rather than the full union-of-upstreams delta, which conflates feature-branch work with default-branch churn.
...
**DIVERGE-4.3** The application shall run `git fetch --no-tags --prune origin` (no explicit refspec, so the remote's configured fetch rules advance every tracked branch) and recompute divergence counts per repository on a 30-second base cadence, doubling the interval for each consecutive fetch failure (capped by `ExponentialBackoff`'s 32× max shift and a 30-minute hard cap, whichever binds first). The prior refspec-scoped shape (`origin <defaultBranch>`) left `origin/<feature>` tracking refs frozen, so polling never surfaced teammate pushes on a linked worktree's own branch; dropping the refspec fixes that alongside `DIVERGE-3.0`'s per-worktree base ref. A fast 5-second polling ticker drives the eligibility check; actual fetches are gated by the per-repo cadence so tracked repositories are not hammered.
...
**DIVERGE-4.6** The polling loop shall also recompute divergence counts for every non-stale worktree on a 30-second per-worktree cadence, independent of the network `git fetch` cadence in DIVERGE-4.3. Local-only recomputation uses no network — `git rev-list`, `git diff --shortstat`, and `git status --porcelain` all run against the local object store — so it catches local changes (a `git add` in an external shell, a commit made by a tool other than Graftty) even on ticks where the repo's fetch cooldown is still active. When a tick finds a per-repo fetch is due in the same cycle, the per-worktree cadence is skipped for that repo because the fetch handler itself recomputes every worktree on success.
