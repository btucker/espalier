...
- **Smart detection:** If the user adds a worktree directory (rather than a repo root), Espalier traces back to the parent repository and adds the full repo with all its worktrees. The dropped worktree is auto-selected.
...
- Responsibilities:
  ...
  - Setting `ESPALIER_SOCK` environment variable on each surface for CLI discoverability
...
When a repo is added, Espalier runs `git worktree list --porcelain` against the repo's git directory. This returns every worktree's path, HEAD commit, and branch name in a machine-parseable format. The repo's own working directory is the first entry.
...
A small standalone binary distributed inside the app bundle at `Espalier.app/Contents/MacOS/espalier`.

```
espalier notify "Build failed"          # set attention on current worktree
espalier notify --clear                  # clear attention on current worktree
espalier notify "Done" --clear-after 10  # auto-clear after 10 seconds
```
...
- **Transport:** Unix domain socket at `~/Library/Application Support/Espalier/espalier.sock`.
...
`TerminalManager` sets `ESPALIER_SOCK` in each terminal's environment, pointing to the socket path. The CLI reads this variable. Other tools can also write directly to the socket without using the CLI.
...
On first launch (or via menu: Espalier -> Install CLI Tool...), the app offers to create a symlink at `/usr/local/bin/espalier` pointing into the app bundle.

### Error Cases

- App not running: CLI prints "Espalier is not running", exits 1.
...
Ghostty (MIT-licensed) provides several Swift components that map directly to Espalier's needs.

### Direct Reuse

| Ghostty Source | Espalier Usage |
|----------------|----------------|
...
- **Rendering:** Metal (managed by libghostty, no Metal code in Espalier)
- **File watching:** FSEvents via `DispatchSource.makeFileSystemObjectSource`
- **IPC:** Unix domain socket (app listens, CLI connects)
- **Persistence:** JSON file in `~/Library/Application Support/Espalier/`
