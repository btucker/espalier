...
├── Package.swift
├── Sources/
│   ├── EspalierKit/                          # Shared library (model, git, notification)
│   │   ├── Model/
...
│   ├── Espalier/                             # macOS app executable
│   │   ├── EspalierApp.swift                 # @main SwiftUI App entry point
│   │   ├── Terminal/
│   │   │   ├── GhosttyBridge.swift           # Swift wrappers for ghostty_app_t, ghostty_config_t
│   │   │   ├── SurfaceHandle.swift           # Wraps ghostty_surface_t + NSView
│   │   │   └── TerminalManager.swift         # Surface lifecycle, focus, ESPALIER_SOCK
│   │   └── Views/
...
**Key decisions:**
- `EspalierKit` is a library shared by both the app and CLI (model types, git logic, socket protocol)
- The app target (`Espalier`) holds everything that depends on AppKit/SwiftUI/libghostty
- The CLI target (`EspalierCLI`) is a lightweight executable depending only on `EspalierKit` and Foundation
- Tests cover `EspalierKit` only — the app and CLI are tested manually and via integration tests
...
- Create: `Sources/EspalierKit/Model/TerminalID.swift` (placeholder to make target compile)
- Create: `Sources/Espalier/EspalierApp.swift` (minimal @main)
- Create: `Sources/EspalierCLI/CLI.swift` (minimal main)
- Create: `Tests/EspalierKitTests/Model/SplitTreeTests.swift` (placeholder)
...
`Sources/EspalierKit/Model/TerminalID.swift`:
```swift
import Foundation
...
`Tests/EspalierKitTests/Model/SplitTreeTests.swift`:
```swift
import Testing
...
Run: `cd /Users/btucker/projects/espalier && swift build 2>&1 | tail -5`
...
git commit -m "feat: scaffold project with EspalierKit, app, and CLI targets"
```
...
**Files:**
- Create: `Sources/EspalierKit/Model/SplitTree.swift`
- Modify: `Tests/EspalierKitTests/Model/SplitTreeTests.swift`
...
`Sources/EspalierKit/Model/SplitTree.swift`:
```swift
import Foundation
...
```bash
git add Sources/EspalierKit/Model/SplitTree.swift Tests/EspalierKitTests/Model/SplitTreeTests.swift
...
**Files:**
- Modify: `Sources/EspalierKit/Model/TerminalID.swift` (already exists, may need tweaks)
- Create: `Sources/EspalierKit/Model/Attention.swift`
- Create: `Sources/EspalierKit/Model/WorktreeEntry.swift`
- Create: `Sources/EspalierKit/Model/RepoEntry.swift`
- Create: `Tests/EspalierKitTests/Model/WorktreeEntryTests.swift`

- [ ] **Step 1: Write failing tests for model types**

`Tests/EspalierKitTests/Model/WorktreeEntryTests.swift`:
```swift
import Testing
import Foundation
@testable import EspalierKit
...
`Sources/EspalierKit/Model/Attention.swift`:
```swift
import Foundation
...
`Sources/EspalierKit/Model/WorktreeEntry.swift`:
```swift
import Foundation
...
`Sources/EspalierKit/Model/RepoEntry.swift`:
```swift
import Foundation
...
```bash
git add Sources/EspalierKit/Model/ Tests/EspalierKitTests/Model/WorktreeEntryTests.swift
...
**Files:**
- Create: `Sources/EspalierKit/Model/AppState.swift`
- Create: `Tests/EspalierKitTests/Model/AppStateTests.swift`
...
`Sources/EspalierKit/Model/AppState.swift`:
```swift
import Foundation
...
```bash
git add Sources/EspalierKit/Model/AppState.swift Tests/EspalierKitTests/Model/AppStateTests.swift
...
**Files:**
- Create: `Sources/EspalierKit/Git/GitRepoDetector.swift`
- Create: `Tests/EspalierKitTests/Git/GitRepoDetectorTests.swift`

- [ ] **Step 1: Write failing tests**

`Tests/EspalierKitTests/Git/GitRepoDetectorTests.swift`:
```swift
import Testing
import Foundation
@testable import EspalierKit
...
`Sources/EspalierKit/Git/GitRepoDetector.swift`:
```swift
import Foundation
...
```bash
git add Sources/EspalierKit/Git/GitRepoDetector.swift Tests/EspalierKitTests/Git/GitRepoDetectorTests.swift
...
**Files:**
- Create: `Sources/EspalierKit/Git/GitWorktreeDiscovery.swift`
- Create: `Tests/EspalierKitTests/Git/GitWorktreeDiscoveryTests.swift`

- [ ] **Step 1: Write failing tests**

`Tests/EspalierKitTests/Git/GitWorktreeDiscoveryTests.swift`:
```swift
import Testing
import Foundation
@testable import EspalierKit
...
`Sources/EspalierKit/Git/GitWorktreeDiscovery.swift`:
```swift
import Foundation
...
**Files:**
- Create: `Sources/EspalierKit/Git/WorktreeMonitor.swift`
...
`Sources/EspalierKit/Git/WorktreeMonitor.swift`:
```swift
import Foundation
...
```bash
git add Sources/EspalierKit/Git/WorktreeMonitor.swift
...
**Files:**
- Create: `Sources/EspalierKit/Notification/NotificationMessage.swift`
- Create: `Sources/EspalierKit/Notification/SocketServer.swift`
- Create: `Tests/EspalierKitTests/Notification/NotificationMessageTests.swift`
- Create: `Tests/EspalierKitTests/Notification/SocketIntegrationTests.swift`

- [ ] **Step 1: Write failing tests for message types**

`Tests/EspalierKitTests/Notification/NotificationMessageTests.swift`:
```swift
import Testing
import Foundation
@testable import EspalierKit
...
`Sources/EspalierKit/Notification/NotificationMessage.swift`:
```swift
import Foundation
...
`Tests/EspalierKitTests/Notification/SocketIntegrationTests.swift`:
```swift
import Testing
import Foundation
@testable import EspalierKit
...
struct SocketIntegrationTests {
    ...
    @Test func serverReceivesMessage() async throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("espalier-sock-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
...
`Sources/EspalierKit/Notification/SocketServer.swift`:
```swift
import Foundation
...
```bash
git add Sources/EspalierKit/Notification/ Tests/EspalierKitTests/Notification/
...
Build the `espalier` CLI binary with `notify` subcommand, worktree resolution, and socket client.

**Files:**
- Create: `Sources/EspalierCLI/WorktreeResolver.swift`
- Create: `Sources/EspalierCLI/SocketClient.swift`
- Modify: `Sources/EspalierCLI/CLI.swift`

- [ ] **Step 1: Implement WorktreeResolver**

`Sources/EspalierCLI/WorktreeResolver.swift`:
```swift
import Foundation
import EspalierKit
...
enum CLIError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .appNotRunning:
            return "Espalier is not running"
        case .socketTimeout:
...
`Sources/EspalierCLI/SocketClient.swift`:
```swift
import Foundation
import EspalierKit

enum SocketClient {
    /// Send a notification message to the Espalier app via Unix domain socket.
    ...
    private static func resolveSocketPath() -> String {
        if let envPath = ProcessInfo.processInfo.environment["ESPALIER_SOCK"] {
            return envPath
        }
...
struct Notify: ParsableCommand {
    static let configuration = CommandConfiguration(
        abstract: "Send an attention notification to Espalier"
    )
...
Run: `swift build --product espalier 2>&1 | tail -5`
Expected: Build succeeds

- [ ] **Step 5: Verify CLI shows help**

Run: `swift run espalier --help 2>&1`
Expected: Shows usage with `notify` subcommand listed

- [ ] **Step 6: Commit**

```bash
git add Sources/EspalierCLI/
git commit -m "feat: add espalier CLI with notify subcommand and socket client"
```
...
Create Swift wrappers around the core libghostty C types. These adapt Ghostty's own wrapper patterns for Espalier's architecture.
...
**Files:**
- Create: `Sources/Espalier/Terminal/GhosttyBridge.swift`
...
`Sources/Espalier/Terminal/GhosttyBridge.swift`:
```swift
import Foundation
import GhosttyKit
...
/// There is exactly one GhosttyApp instance per Espalier process.
...
```bash
git add Sources/Espalier/Terminal/GhosttyBridge.swift
...
**Files:**
- Create: `Sources/Espalier/Terminal/SurfaceHandle.swift`
- Create: `Sources/Espalier/Terminal/TerminalManager.swift`

- [ ] **Step 1: Implement SurfaceHandle**

`Sources/Espalier/Terminal/SurfaceHandle.swift`:
```swift
import AppKit
import GhosttyKit
import EspalierKit
...
final class SurfaceHandle {
    ...
    init(
        ...
        // Set ESPALIER_SOCK environment variable
...
`Sources/Espalier/Terminal/TerminalManager.swift`:
```swift
import AppKit
import GhosttyKit
import EspalierKit
...
```bash
git add Sources/Espalier/Terminal/SurfaceHandle.swift Sources/Espalier/Terminal/TerminalManager.swift
...
**Files:**
- Create: `Sources/Espalier/Views/SurfaceViewWrapper.swift`
- Create: `Sources/Espalier/Views/SplitContainerView.swift`
- Create: `Sources/Espalier/Views/TerminalContentView.swift`

- [ ] **Step 1: Implement SurfaceViewWrapper**

`Sources/Espalier/Views/SurfaceViewWrapper.swift`:
```swift
import SwiftUI
import AppKit
...
`Sources/Espalier/Views/SplitContainerView.swift`:
```swift
import SwiftUI
import EspalierKit
...
`Sources/Espalier/Views/TerminalContentView.swift`:
```swift
import SwiftUI
import EspalierKit
...
```bash
git add Sources/Espalier/Views/SurfaceViewWrapper.swift Sources/Espalier/Views/SplitContainerView.swift Sources/Espalier/Views/TerminalContentView.swift
...
**Files:**
- Create: `Sources/Espalier/Views/WorktreeRow.swift`
- Create: `Sources/Espalier/Views/SidebarView.swift`
- Create: `Sources/Espalier/Views/BreadcrumbBar.swift`
- Create: `Sources/Espalier/Views/MainWindow.swift`
...
`Sources/Espalier/Views/BreadcrumbBar.swift`:
```swift
import SwiftUI
import EspalierKit
...
```bash
git add Sources/Espalier/Views/
...
- [ ] **Step 1: Add pane navigation commands to EspalierApp**

In `Sources/Espalier/EspalierApp.swift`, add to the `.commands` block inside the `CommandGroup(after: .newItem)` section:
...
- [ ] **Step 2: Add navigation helper methods to EspalierApp**

Add these methods to EspalierApp:
...
In `Sources/Espalier/Views/MainWindow.swift`, update `selectWorktree` to handle stop with confirmation. The `SidebarView.stopWorktree` method should post a notification that `MainWindow` handles with a confirmation dialog:
...
```bash
git add Sources/Espalier/EspalierApp.swift Sources/Espalier/Views/MainWindow.swift
...
```bash
echo '{"type":"notify","path":"<worktree-path>","text":"Build failed"}' | nc -U ~/Library/Application\ Support/Espalier/espalier.sock
```
